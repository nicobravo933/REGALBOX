...
import {
  Star, ArrowRight, ChevronLeft, ChevronRight, Heart,
  ...
  return (
    <section className="relative overflow-hidden min-h-[520px] md:min-h-[600px] flex items-center">
      ...
      <div className="relative max-w-6xl mx-auto px-4 py-16 w-full">
        ...
        >
          <span style={{ color: 'var(--foreground)', fontWeight: 700 }}>Regalos que</span><br />
          <em style={{ color: 'var(--primary)', fontStyle: 'italic', fontWeight: 700 }}>emocionan</em>
        </h1>
        <p className="hidden md:block mb-6 max-w-sm leading-relaxed text-muted-foreground" style={{ fontSize: '0.95rem' }}>
          Creamos experiencias únicas con flores, cajas personalizadas y más. Para cada ocasión especial.
        </p>
        ...
        <div className="flex gap-3 mt-9 flex-wrap">
          {[
            ...
            <div
              ...
              style={{ background: 'rgba(255,255,255,0.75)', border: '1px solid rgba(139,61,69,0.15)', backdropFilter: 'blur(4px)' }}
            >
              <p style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '1.3rem', color: 'var(--primary)' }}>{s.value}</p>
              <p style={{ fontSize: '0.72rem', color: 'var(--muted-foreground)', marginTop: '2px' }}>{s.label}</p>
            </div>
          ))}
        </div>
      </div>
...
function Categories() {
  const cats = [
    { id: 'romantico', label: 'Romántico', image: '/images/products/ramo1.jpeg', href: '/tienda?cat=romantico' },
    { id: 'cumpleanos', label: 'Amistad', image: '/images/products/corazon1.jpeg', href: '/tienda?cat=cumpleanos' },
    { id: 'aniversario', label: 'Cumpleaños', image: '/images/products/stich1.jpeg', href: '/tienda?cat=aniversario' },
    { id: 'personalizado', label: 'Personalizado', image: '/images/products/oso1.jpeg', href: '/tienda?cat=personalizado' },
  ];
...
function Features() {
  const feats = [
    { icon: <Heart className="w-5 h-5" />, title: 'Hecho con amor', desc: 'Cada regalo es preparado a mano con dedicación.' },
    { icon: <Truck className="w-5 h-5" />, title: 'Envío express', desc: 'Entregamos en el día para Buenos Aires.' },
    { icon: <Package className="w-5 h-5" />, title: 'Box a medida', desc: 'Personalizamos cada detalle para ti.' },
  ];
      ...
      <div className="max-w-6xl mx-auto px-4 grid grid-cols-1 sm:grid-cols-3 gap-4">
        {feats.map(f => (
          <div key={f.title} className="flex items-start gap-3 p-4">
            ...
            <div>
              <p className="font-medium text-foreground text-sm" style={{ fontFamily: 'var(--font-display)' }}>{f.title}</p>
...
function BestSellers() {
  ...
  const bestsellers = products.filter(p => p.badge === 'Más vendido' || p.badge === 'Popular' || p.rating >= 4.9).slice(0, 6);
    ...
    <section style={{ background: 'var(--primary)' }} className="py-16">
      <div className="max-w-6xl mx-auto px-4 text-center">
        ...
        <p className="text-white/80 mb-8 max-w-md mx-auto">
          Elegí los productos que más te gustan y creamos un box personalizado. Primeras 10 cajas con 20% de descuento.
        </p>
...
const reviews = [
  {
    ...
    avatar: 'MF',
    rating: 5,
    text: 'El bouquet que recibí fue increíble. Las flores llegaron frescas y el packaging era hermoso. ¡Mi pareja quedó sin palabras!',
  ...
  },
  {
    ...
    avatar: 'JL',
    rating: 5,
    text: 'Pedí el box a medida para el cumpleaños de mi mamá y superó todas mis expectativas. Super recomendado.',
    product: 'Box Personalizado',
    date: 'hace 1 semana',
  },
  {
    id: 3,
    ...
    avatar: 'SM',
    rating: 5,
    text: 'Atención excelente, entrega puntual y el producto exactamente como en las fotos. Ya hice mi tercer pedido.',
    product: 'Rosas Premium',
    date: 'hace 2 semanas',
  },
  {
    id: 4,
    ...
    avatar: 'DR',
    rating: 4,
    text: 'Muy buen servicio. El arreglo floral quedó perfecto para el aniversario. El envío fue rápido.',
    product: 'Arreglo Aniversario',
    date: 'hace 3 semanas',
  },
];
      ...
      <div className="max-w-6xl mx-auto px-4">
        <div className="text-center mb-8">
          ...
          <div className="flex items-center justify-center gap-1 mt-2">
            ...
            <span className="text-muted-foreground text-sm ml-2">4.9 de 5 — más de 500 reseñas</span>
          </div>
        </div>
        ...
        <div className="md:hidden relative">
          <div className="bg-card rounded-2xl p-6 shadow-sm border border-border mx-4">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <p className="font-medium text-sm text-foreground">{reviews[current].name}</p>
                <p className="text-xs text-muted-foreground">{reviews[current].date}</p>
              </div>
            </div>
            <div className="flex gap-0.5 mb-2">
              {[1, 2, 3, 4, 5].map(n => (
                <Star key={n} className={`w-3.5 h-3.5 ${n <= reviews[current].rating ? 'fill-accent text-accent' : 'text-border'}`} />
              ))}
            </div>
            <p className="text-muted-foreground text-sm">{reviews[current].text}</p>
            <p className="text-xs text-primary mt-2">Compró: {reviews[current].product}</p>
          </div>
          <div className="flex justify-center gap-3 mt-4">
            <button type="button" onClick={prev} className="w-8 h-8 rounded-full border border-border flex items-center justify-center hover:bg-muted">
              <ChevronLeft className="w-4 h-4" />
            </button>
            ...
            <button type="button" onClick={next} className="w-8 h-8 rounded-full border border-border flex items-center justify-center hover:bg-muted">
              <ChevronRight className="w-4 h-4" />
            </button>
          </div>
        </div>
        ...
        <div className="hidden md:grid grid-cols-2 gap-4">
          {reviews.map(r => (
            <div key={r.id} className="bg-card rounded-2xl p-6 shadow-sm border border-border">
              <div className="flex items-center gap-3 mb-3">
              ...
              <div className="flex gap-0.5 mb-2">
                {[1, 2, 3, 4, 5].map(n => (
                  <Star key={n} className={`w-3.5 h-3.5 ${n <= r.rating ? 'fill-accent text-accent' : 'text-border'}`} />
                ))}
              </div>
      ...
      <div className="grid md:grid-cols-2 gap-10 items-center">
        ...
        <div>
          ...
          <p className="text-muted-foreground mb-4 leading-relaxed">
            ...
            Desde 2020 empaquetamos amor, cuidado y creatividad en cada box.
          </p>
          <p className="text-muted-foreground mb-6 leading-relaxed">
            Cada producto es seleccionado a mano, cada caja es armada con dedicación,
            y cada entrega lleva una tarjeta personalizada porque creemos que los detalles marcan la diferencia.
          </p>
          <div className="grid grid-cols-3 gap-4">
            {[
              ...
              <div key={s.label} className="text-center bg-secondary rounded-2xl p-4">
                <p className="text-primary font-bold text-xl" style={{ fontFamily: 'var(--font-display)' }}>{s.value}</p>
  ...
  return (
    <section id="contacto" className="py-12 max-w-6xl mx-auto px-4">
      <div className="grid md:grid-cols-2 gap-10 items-start">
        <div>
          <h2 className="text-foreground mb-3" style={{ fontFamily: 'var(--font-display)' }}>Envianos un mensaje</h2>
          <p className="text-muted-foreground mb-6">
            ¿Tenés alguna consulta o querés armar un pedido especial? Estamos para ayudarte.
          </p>
          <div className="flex flex-col gap-3">
            {[
              { icon: '📍', label: 'Dirección', value: 'Av. Santa Fe 1234, CABA, Argentina' },
        ...
        <div className="bg-card rounded-2xl p-6 border border-border shadow-sm">
          {sent ? (
            <div className="text-center py-8">
              ...
              <h3 style={{ fontFamily: 'var(--font-display)', color: 'var(--primary)' }}>¡Mensaje enviado!</h3>
              <p className="text-muted-foreground text-sm mt-2">Te contactaremos a la brevedad.</p>
              <button
                type="button"
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              <div>
                <label htmlFor="contact-name" className="block text-sm text-muted-foreground mb-1">Nombre</label>
                <input
                  id="contact-name"
                  required
                  value={form.name}
              ...
              <div>
                <label htmlFor="contact-email" className="block text-sm text-muted-foreground mb-1">Email</label>
                <input
                  id="contact-email"
                  required
                  type="email"
              ...
              <div>
                <label htmlFor="contact-message" className="block text-sm text-muted-foreground mb-1">Mensaje</label>
                <textarea
                  id="contact-message"
                  required
                  rows={4}
